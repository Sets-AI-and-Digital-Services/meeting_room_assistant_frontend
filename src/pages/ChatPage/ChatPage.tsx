...
export default function ChatPage() {
  ...
  // Next step-friendly: keep selected room
  const [selectedRoomId, setSelectedRoomId] = useState<string | null>(null);

  const onSelectRoom = useCallback((roomId: string) => {
    setSelectedRoomId(roomId);
    console.log("Selected room:", roomId);
    // Next step: open calendar panel and fetch room calendar by roomId
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#070A12] text-white">
      ...
      <div className="relative max-w-6xl px-6 py-8 mx-auto">
        {/* header */}
        <motion.header
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="leading-tight">
              <div className="text-sm text-white/60">Meeting Room Assistant</div>
              <div className="text-lg font-semibold tracking-tight">
                Bank Scheduler{" "}
                <span className="ml-2 rounded-full bg-amber-400/10 px-2 py-0.5 text-xs text-amber-200 ring-1 ring-amber-300/15">
                  Enterprise
                </span>
              </div>
            </div>
          ...
          <div className="flex items-center gap-3 px-4 py-2 rounded-2xl bg-white/5 ring-1 ring-white/10">
            <span
              className={`h-2 w-2 rounded-full ${
                isReady ? "bg-emerald-500" : hasError ? "bg-rose-500" : "bg-amber-500"
              }`}
            />
        ...
        <div className="grid grid-cols-1 gap-6 mt-10 lg:grid-cols-12">
          ...
          >
            <div className="rounded-3xl bg-white/5 p-7 ring-1 ring-white/10 backdrop-blur">
              <div className="inline-flex items-center gap-2 px-3 py-1 text-xs rounded-full bg-emerald-500/10 text-emerald-200 ring-1 ring-emerald-300/15">
              ...
              <p className="mt-4 text-white/70">
                Ask naturally and the assistant will check availability, suggest the best room,
                and open the room calendar.
              </p>

              <div className="w-full h-px mt-7 bg-gradient-to-r from-emerald-400/40 via-amber-300/25 to-transparent" />

              <div className="flex flex-wrap gap-2 mt-5">
                {["Capacity-aware", "Conference-ready", "Conflict-safe", "Fast suggestions"].map((t) => (
                  <span
                    key={t}
                    className="px-3 py-1 text-xs rounded-full bg-white/5 text-white/70 ring-1 ring-white/10"
                  >
                    {t}
                  </span>
                ))}
              ...
              {/* Tiny UX hint showing selection (optional, safe) */}
              {selectedRoomId ? (
                <div className="p-4 mt-6 text-sm rounded-2xl bg-emerald-500/10 text-emerald-100 ring-1 ring-emerald-300/15">
                  Selected room: <span className="font-semibold">{selectedRoomId}</span>
                  <div className="mt-1 text-xs text-emerald-100/70">
                    Next step: show this room’s calendar.
                  </div>
                </div>
              ) : null}
            </div>
            ...
            <ChatShell
              header={
                <div className="flex items-center justify-between px-6 py-4">
                  ...
                  <div className="px-3 py-1 text-xs rounded-full bg-amber-400/10 text-amber-200 ring-1 ring-amber-300/15">
                    Secure
                  </div>
                </div>
              }
        ...
        {/* footer */}
        <div className="flex items-center justify-between mt-8 text-xs text-white/35">
